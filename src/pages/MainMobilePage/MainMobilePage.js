...
import { FilterFilled, ShoppingFilled } from '@ant-design/icons';
...
import PreloaderComponent from '../../components/atoms/PreloaderComponent/PreloaderComponent';
...
class MainDesktopPage extends Component {
  ...
  // function to set filter components visible
  onClickFilter = () => {
    if (this.state.filterVisible) {
      this.onInit("filterVisible", false)
    } else {
      this.onInit("filterVisible", true)
    }
  }

  // function to use when shopping cart modal needs to be displayed
  showDisplayCartModal = () => {
    this.onInit("cartModalVisible", true)
  }

  // function to hide shopping cart modal
  hideDisplayCartModal = () => {
    this.onInit("cartModalVisible", false)
  }
  ...
  render() {
    ...
    return (
      <StyledBody>
        {loading ? (
          <PreloaderComponent height={16} />
        ) : (
          <>
            <Header state={this.state} headerMargin={120} cartMargin={0} />
            <Row style={{ marginTop: 20 }}>

              <Col offset={4} span={16} ><ButtonComponent icon={<FilterFilled />} text="Filter" onClick={this.onClickFilter} /></Col>
              {this.state.filterVisible ? (
                <Col xs={24} sm={24} md={24} lg={6}>
                  <Row style={{ marginLeft: 40 }}>
                    <Col span={24} style={{ marginTop: 20 }}>
                      <Text style={{ fontSize: "16px", fontWeight: 600 }}>Sorting Mobile</Text>
                    </Col>
                    <Col>
                      <SortingComponent
                        state={this.state}
              ...
              <Col offset={4} span={16} style={{ marginTop: 20 }}><ButtonComponent icon={<ShoppingFilled />} text="Display Cart" onClick={this.showDisplayCartModal} />
                <Modal
                  title="Shopping Cart"
                  visible={this.state.cartModalVisible}
                  onCancel={this.hideDisplayCartModal}
                  footer={[
                    <Row align="middle" justify="end" gutter={[20, 20]}>
                      <Col>
                        <ButtonComponent size="default" text="Close" onClick={this.hideDisplayCartModal} />
                      </Col>
                    </Row>
                  ]}
              ...
              <Col xs={24} sm={24} md={24} lg={12}>
                <Row gutter={[0, 10]} style={{ marginLeft: 70 }}>
